            ...
            for scale in np.linspace(1 - scale_index, 1 + scale_index, using_scale)[::-1]:
                ...
                for img in images:
                    ...
                    if using_weights:
                        ...
                        if i == 0:  # left
                            # update the min/max if it's the first cycle or if the weighted val is less/more than the
                            # previous weight the val using the position, in the left case, the more the point is at
                            # left, the more is weighted (so if loc[0] -> 0 (left side) the value must increase,
